...
def run_code():
    ...
    with open('./input.txt') as f:
      p = Popen(cmd, shell=True, stdin=f, stdout=PIPE,
                stderr=STDOUT, close_fds=True)
    # p = Popen(cmd, shell=True, stdin=PIPE, stdout=PIPE,
    #           stderr=STDOUT, close_fds=True)
    timeout=5
    print(time.time()-start_time)
    while timeout and p.poll() == None:
        if time.time()-start_time >= 3:
            ...
            return jsonify("Time limit exceeded")
...
def submit_code():
    ...
    g.write(output.decode('utf-8'))
    g.close()
    print(filecmp.cmp("output.txt","demofile3.txt" ))
    return jsonify("Passed" if filecmp.cmp("output.txt","demofile3.txt" ) else "Failed")
    # print("12")
    # from code_checker import codechecker
    # print("sub")
    # codechecker(
    #     filename=session["file_name"],               # Source code file
